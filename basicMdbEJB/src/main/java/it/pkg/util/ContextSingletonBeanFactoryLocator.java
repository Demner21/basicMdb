...
public class ContextSingletonBeanFactoryLocator extends SingletonBeanFactoryLocator {
  ...
  protected BeanFactory createDefinition(String resourceLocation, String factoryKey) {
    ...
    final ClassPathXmlApplicationContext objAplicationContext = new ClassPathXmlApplicationContext(
  ...
  /**
   * Returns an instance which uses the default
   * "classpath*:beanEjbSpringRefContext.xml", as the name of the definition
   * file(s). All resources returned by the current thread's context classloader's
   * getResources() method with this name will be combined to create a definition,
   * which is just a BeanFactory.
   */
   ...
   * class loader's <code>getResources</code> method will be called with this
   * value to get all resources having that name. These resources will then be
   * combined to form a definition. In the case where the name uses a Spring
   * "classpath:" prefix, or a standard URL prefix, then only one resource file
   * will be loaded as the definition.
   * 
   * @param selector
   *          the name of the resource(s) which will be read and combine to form
   *          the definition for the SingletonBeanFactoryLocator instance. The one
   *          file or multiple fragments with this name must form a valid
  ...
  /**
   * Constructor which uses the default "classpath*:beanEjbSpringRefContext.xml",
   * as the name of the definition file(s). All resources returned by the
   * definition classloader's getResources() method with this name will be
   * combined to create a definition.
   */
  ...
  /**
   * Constructor which uses the the specified name as the name of the definition
   * file(s). All resources returned by the definition classloader's
   * getResources() method with this name will be combined to create a definition.
   */
   ...
   * assumes is a ConfigurableApplicationContext. This implementation simply
   * invokes {@link ConfigurableApplicationContext#refresh
  ...
  protected void destroyDefinition(BeanFactory objBeanFactory, String nombreRecurso) throws BeansException {
    if (objBeanFactory instanceof ConfigurableApplicationContext) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("ContextSingletonBeanFactoryLocator group with resourceName=['" + nombreRecurso
